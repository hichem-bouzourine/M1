...
public class Outils {
	...
	/**
	 * ! 20% dernier : Calcule et sauvegarde les temps d'exécution de l'ajout d'une
	...
	/**
	 * ! 20% dernier : Calcule et sauvegarde les temps d'exécution de la suppression
	...
	public static void AjoutFileBinomiale() {
		...
		for (int i = 1; i <= nb; i++) {
			for (int j = 0; j < tailles.length; j++) {
				...
				for (int k = 0; k < cles.size(); k++) {
					ICle c = cles.get(k);
					// Mettre 80 % du dataset dans une file et calculer le temps nécessaire pour
					// les 20 % restants
					if (k < cles.size() - (cles.size() / 20))
						file = (FileBinomiale) file.ajout(new TournoiBinomial(c));

					// Pour les 20% des ajouts restant
					// Il faut calculer le temps de l'ajout
					else {
						debut = System.nanoTime();
