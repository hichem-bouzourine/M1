...
// Fonction globale
globalFunDef returns [com.paracamplus.ilp2.partiel2016.interfaces.IASTfunctionDefinitionOpt node]
    : 'function' name=IDENT '(' 
        vars+=IDENT? (',' vars+=IDENT)* 
        (keys='&keys' nvars+=IDENT vals+=expr 
        ( ',' nvars+=IDENT vals+=expr)*  )?
        ')' body=expr
    ;
