...
// expressions de la grammaire précédente
    ...
    | 'letrange' vars+=IDENT ':' l+=expr '..' h+=expr '='  vals+=expr 
      ('and' vars+=IDENT ':' l+=expr '..' h+=expr '=' vals+=expr)*
      'in' body=expr #BlockRange
    
    // FOR EACH example

    // | 'foreach' var=IDENT  'in'  arr=expr
    //   body=expr # forEach
