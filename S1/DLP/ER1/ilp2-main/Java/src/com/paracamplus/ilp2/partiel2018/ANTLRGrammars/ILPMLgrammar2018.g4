...
prog
	returns[com.paracamplus.ilp2.interfaces.IASTprogram node]: (
		defs += globalFunDef ';'?
	)* (exprs += expr ';'?)* EOF;

// Fonction globale
globalFunDef
	returns[com.paracamplus.ilp2.partiel2018.interfaces.IASTcountingFunctionDefinition node]:
	'function' (count = ':comp')? name = IDENT '(' vars += IDENT? (
		',' vars += IDENT
	)* ')' body = expr;

// Expressions enrichies
expr
	returns[com.paracamplus.ilp1.interfaces.IASTexpression node]:
	...
	| var = IDENT												# Variable
	| 'let' vars += IDENT '=' vals += expr (
		'and' vars += IDENT '=' vals += expr
 ...
 * Règles lexicales.
 * 
 * ANTLR impose que le nom des règles lexicales commencent par une majuscule. Ces règles prennent la
